    ...
    <div className="rounded-xl border bg-surface p-4">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-xs font-medium tracking-wide text-muted uppercase">SOP verdict</div>
          <p className="mt-1 text-[13px] text-fg">{verdict?.summary ?? "No frame analyzed yet."}</p>
        </div>
        {verdict && (
          <div className="shrink-0 text-right">
            ...
            <div className="text-[10px] tracking-wide text-muted uppercase">score</div>
          </div>
        )}
      </div>
      ...
      {verdict && verdict.violations.length > 0 && (
        <div className="mt-3 space-y-1.5">
          <div className="text-xs font-medium tracking-wide text-muted uppercase">Violations</div>
            ...
            return (
              <div key={`${v.ruleId}-${i}`} className="rounded-lg border bg-panel p-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[13px] font-medium">{rule?.label ?? v.ruleId}</span>
                </div>
      ...
      {ledger && (
        <div className="mt-3 flex items-center gap-2 border-t pt-3 text-[11px] text-muted">
          ...
          <span className="font-mono">sealed · {ledger.hash.slice(0, 16)}…</span>
        </div>
      )}
    </div>
  );
